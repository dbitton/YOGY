...
use vars qw($TMPURI $TMPDIR $all_go_ids
            ...
            %inparanoid_species %inparanoid_org_names %function_colour);
    ...
    if ($cgi->param() ) {
        ...
        if ($name =~ /^([OPQ][\d][A-Z\d]{3}\d)/) {
            ...
            if ($species eq 'S._pombe') {
                ...
                $tmp_id = get_ens_from_uni($dbh, $uni_query);
        ...
        #if ($name =~ /^(Hs)?\d+$/ and ($species ne 'M._musculus' or $species ne 'R._norvegicus') ) {
        if ($name =~ /^(Hs)?(\d+)$/) {

            my $tax_id = $species_tax_rev{$species_id{$species} };

            my $tmp_id = get_id_from_gi($dbh, $2, $tax_id);
            ...
            if (defined $tmp_id and $tmp_id) {
                ...
                my $tmp_id = get_ens_from_gi($dbh, $gi_query);

                $ens_query = $tmp_id
                    if defined $tmp_id and $tmp_id;

                #print "[$name] [$ens_query]\n";
        ...
        if ($name =~ /^ENS/ and $species eq 'H._sapiens') {

            $gene_info = get_info_from_ens($dbh, $name);
            ...
            #my $gene_id = get_id_from_gi($dbh, $gi_query);

            #print "[$yogy_id] [$gene_id]\n";
...
sub do_summary {
    ...
    if ($species eq 'S._pombe') {
        ...
        if ($sys_id =~ /^ENS/) {
            ...
            $href  = "http://apr2005.archive.ensembl.org/$ensembl_species{$species_id}/geneview?peptide=$sys_id";
            $title = 'Ensembl';
    ...
    if ($ens_query and $sys_id ne $ens_query) {

        $href  = "http://apr2005.archive.ensembl.org/$ensembl_species{$species_id}/geneview?peptide=$ens_query";
 ...
 PROT:
    ...
    PID:
        ...
        if ($prot->[2] =~ /^ens/) {
            ...
            my $gene_info = get_info_from_ens($dbh, $search_id);
 ...
 PROT:
    foreach my $prot (@orthomcl_results) {
        ...
        if ($prot->[2] eq 'Aga' or
            ...
            $href  = "http://apr2005.archive.ensembl.org/$ensembl_species{$species_id}/geneview?peptide=$search_id";
            $title = 'Ensembl';
...
#            $title = 'Gramene';
            ...
            $href  = "http://www.arabidopsis.org/servlets/TairObject?type=gene&name=At5g35980.1";
            $title = 'TAIR';

        } elsif ($prot->[2] eq 'Ago' or
                 $prot->[2] eq 'Cgl' or
                 $prot->[2] eq 'Kla') {
        ...
        my $href2 = "http://www.expasy.org/cgi-bin/niceprot.pl?$uni_id";
        my $href3 = "http://orthomcl.cbil.upenn.edu/cgi-bin/OrthoMclWeb.cgi?rm=sequence&accession=$prot->[3]";
...
#            keys %{ { map { $_->[0], 1 } @go_ids} };
    ...
    foreach my $gid (@go_ids) {
        ...
        push @{$go_ids}, [ $gid->[0], $search_go, $gid->[1] ];
...
sub save_go_info {
    ...
    foreach my $arr (@{$go_ids}) {
        ...
        push
            @{$all_go_ids->{$go_id}{$species_3let{$species_id} } },
            [$prot_id, $qual];
            ...
            foreach my $lid (sort @{$local_search_gos}) {
                ...
                if (lc $mid->[0] eq $lid) {
                    ...
                    if ($mid->[1] eq 'NOT') {
                        ...
                        $local_string .= ' <font color="#F75D59">[NOT]</font>';
...
sub print_final_go_table {
 ...
 GID:
    foreach my $go_id (sort keys %{$all_go_ids}) {
        ...
        ($count, $go_org_string) = get_final_go_string($count, $go_id);

        #print "[$count] [$go_org_string] [$go_id]<br />\n";
...
sub get_info_from_ens {
    ...
    my $fgene = (split /;/, $row[13])[0];

    my $gene_id = (split /,/, $fgene)[0];

    my $fgi = (split /;/, $row[17])[0];
