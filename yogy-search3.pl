...
use vars qw($TMPURI $TMPDIR $gi_query $all_go_ids
            ...
            %inparanoid_org_names %function_colour);
...
sub main {
    ...
    if ($cgi->param() ) {
        ...
        if ($name =~ /^ENS/) {
            ...
            my $tmp_id = get_id_from_ens($dbh, $name);
...
sub do_summary {
    ...
    if ($data_check[3] eq 'Y') {
        ...
        $img4 = "<a href='#curated' title='Curated results'><img src=$yes_img /></a>";
...
sub print_final_go_table {
 ...
 GID:
    foreach my $go_id (sort keys %{$all_go_ids}) {
        ...
        ($count, $go_org_string) = get_go_string($count, $go_id);
...
#    print "[@row[1,5,6,10,11,12,16]]\n";

#    print "[$db_id] [$row[12]] [] [$row[16]]\n";
