...
%species_hrefs = (Ath => 'http://www.arabidopsis.org/servlets/Search?type=general&search_action=detail&method=1&sub_type=gene&name=',
                  ...
                  Pfa => 'http://www.genedb.org/genedb/Search?organism=malaria&name=',
                  Rno => 'http://rgd.mcw.edu/tools/genes/genes_view.cgi?id=',
                  Sce => 'http://db.yeastgenome.org/cgi-bin/locus.pl?locus=',
                  Spo => 'http://www.genedb.org/genedb/Search?organism=S.+pombe&name=');
...
sub main {
    ...
    if ($cgi->param() ) {
        ...
        my $dbh = connect_to_DB('mysql', 'S_pombe_YOGY_1', 'webdbsrv',
                                '3306', '', 'yogyro', '');
        ...
        if ( ($name =~ /^ENS/ or
            ...
            $ens_query = get_ens_from_ids($dbh, $name, $species_id);
...
sub do_summary {
    ...
    if ($species eq 'S._pombe') {
        ...
        if ($sys_id !~ /^ENS/) {
            ...
            $ens_id = get_ens_from_ids($dbh, $sys_id, $species_id);
    ...
    if ($species eq 'S._pombe') {
        ...
        my $href3 = "http://www.sanger.ac.uk/perl/SPGE/geexview?group=4&q=$new_id&scale=log2&scale_min=-2&scale_max=2";
        my $href2 = "http://www.sanger.ac.uk/perl/SPGE/geexview?group=3&q=$new_id&scale=linear&scale_auto=on";
        my $href1 = "http://www.sanger.ac.uk/perl/SPGE/geexview?group=2&q=$new_id&scale=linear&scale_auto=on";
...
sub do_inparanoid {
 ...
 PROT:
    ...
    my $href1 = "http://inparanoid.cgb.ki.se/cgi-bin/etext_search.pl?terms=$search_inp&species=$search_db";
 ...
 PROT:
    foreach my $prot (@orthomcl_results) {
        ...
        if ($prot->[2] eq 'Aga' or
            ...
            if ($prot->[2] eq 'Mmu' or $prot->[2] eq 'Rno') {
...
sub print_help_bar {
    ...
    print table({-width       => '100%',
                ...
                Tr({-align  => 'center',
                    -valign => 'top',
                    -class  => 'violet3'},
                   [td(['', '<a href="/PostGenomics/S_pombe/YOGY/index.shtml" target=$gBlank ' .
                        "style='$gStyle'>YOGY Home</a>",
                        '', '<a href="/PostGenomics/S_pombe/YOGY/help.shtml" target=$gBlank ' .
...
sub print_form {
    ...
    print table({-width       => '100%',
                ...
                Tr({-align  => 'center',
                    -valign => 'top',
                    -class  => 'violet3'},
                   [td(['', '<a href="mailto:webmaster@sanger.ac.uk" style="' . $gStyle .
                        '">webmaster@sanger.ac.uk</a>', '']) ])
               );
